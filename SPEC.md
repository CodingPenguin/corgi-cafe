...
- `POST messages` body `{ name, text, senderId?, lat?, lng? }` → run presence check; if not allowed return 403 `{ "error": "not-at-cafe" }`. Validate: text non-empty after trim, ≤ 500 chars; name trimmed, default "Anonymous Corgi", ≤ 30 chars; senderId must be a UUID when present. Rate limit: in-memory Map of IP → last post ts, reject < 2000ms apart with 429 `{ "error": "too-fast" }`. On success insert into Supabase (id = crypto.randomUUID()), return `{ "ok": true, "message": <the message> }`, and opportunistically remove messages older than 24 hours. The server sets `is_creator` only after verifying the signed HttpOnly creator cookie.
- `GET claim?token=...` → compare the private claim token against `CORGI_CREATOR_CLAIM_SECRET`, issue a one-year signed HttpOnly cookie using `CORGI_CREATOR_COOKIE_SECRET`, and redirect to `/chat`. A valid creator cookie bypasses the location requirement for presence checks, reading, and posting. Rotate only the claim secret after the creator devices are claimed.
...
- Messages verified as creator posts show a crown beside the display name. Hovering or focusing the crown reveals “Creator.”
