...
- `GET claim?token=...` → compare the private claim token against `CORGI_CREATOR_CLAIM_SECRET`, issue a one-year signed HttpOnly cookie using `CORGI_CREATOR_COOKIE_SECRET`, and redirect to `/chat`. Rotate only the claim secret after the creator devices are claimed.
